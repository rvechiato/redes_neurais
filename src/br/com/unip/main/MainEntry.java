...
public class MainEntry extends JFrame implements Runnable {
	...
	void load_actionPerformed(java.awt.event.ActionEvent event) {
		try {
			...
			f = new FileReader(new File(".\\arquivos\\sample.dat"));
			...
			while ((line = r.readLine()) != null) {
				SampleData ds = new SampleData(line.charAt(0), MainEntry.DOWNSAMPLE_WIDTH, MainEntry.DOWNSAMPLE_HEIGHT);
	...
	void save_actionPerformed(java.awt.event.ActionEvent event) {
		try {
			...
			os = new FileOutputStream(".\\arquivos\\sample.dat", false);
	...
	public void run() {
		try {
			int inputNeuron = MainEntry.DOWNSAMPLE_HEIGHT * MainEntry.DOWNSAMPLE_WIDTH;
